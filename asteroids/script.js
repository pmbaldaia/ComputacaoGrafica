...
function setup() {
  ...
  textFont("Space Corner");
  ship = new Ship();
  initialize("Vamos jogar! Pressione ESPAÇO", initastnum);
}
...
function draw() {
  ...
  if (asteroids.length == 0) { // player cleared the level
    astnum += 3;
    initialize("Ganhou, subiu de nível!", astnum);
  }
  ...
  for (let i = asteroids.length - 1; i >= 0; i--) {
    ...
    if (ship.hit(asteroids[i]) && !ship.safe) {
      ...
      console.log("Dano do escudo" + ship.shieldLevel);
      asteroids[i].explode();
...
Ship.prototype.interface = function() {
  ...
  fill(255);
  noStroke();
  text("Pontuação = " + this.score, 50, 50);
  //text("Shield = " + constrain(round(ship.shieldLevel), 0, 100), 50, 65);
  if (this.shieldLevel >= this.shieldMax) {
    text("Escudo = Máximo!", 50, 65);
  } else {
    text("Escudo = " + constrain(round(this.shieldLevel), 0, round(this.shieldLevel)), 50, 65);
  }
  text("Nível = " + gameLevel, 50, 80);
  if (message) {
    textSize(32);
...
Ship.prototype.getDamage = function(obj) {
  let damount = obj.r; // the bigger the object hitting the ship the heavier the damage amount
...
Ship.prototype.explode = function() {
  let debrisVel = p5.Vector.random2D().mult(random(0.5, 1.5));
  //let debrisVel = p5.Vector.add(this.lasers[i].vel.mult(0.2), asteroids[j].vel);
  let debrisNum = 50;
    ...
    if (this.lasers[i].offscreen()) { 
      ...
      for (let j = asteroids.length - 1; j >= 0; j--) {
        if (this.lasers[i].hits(asteroids[j])) {
          ...
          let newAsteroids = asteroids[j].breakup(); // returns an array of two smaller asteroids
...
function Asteroid(pos, s) {
  ...
  this.angle = 0;
  let increment = map(this.r, this.rmin, this.rmax, 0.1, 0.01);
  if (random() > 0.5) {
